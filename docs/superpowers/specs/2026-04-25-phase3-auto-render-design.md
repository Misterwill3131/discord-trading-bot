...
### 5. Worker — `scripts/video-render-worker.js`
...
1. Lance le worker : `node scripts/video-render-worker.js`
...
4. **Worker boot** : `node scripts/video-render-worker.js` log `[worker] ready, polling http://...` sans erreur.
