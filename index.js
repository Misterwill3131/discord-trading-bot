...
const imageGallery = []; // last 100 generated images
function addToGallery(type, ticker, author, buffer) {
  const id = Date.now() + '-' + Math.random().toString(36).slice(2, 6);
  imageGallery.unshift({ id, type, ticker: ticker || null, author: author || null, ts: new Date().toISOString(), buffer });
  if (imageGallery.length > 100) imageGallery.pop();
  return id;
}
...
app.get('/ticker/:symbol', requireAuth, (req, res) => {
  res.set('Content-Type', 'text/html');
  res.send(TICKER_PAGE_HTML);
});
...
app.get('/api/gallery', requireAuth, (req, res) => {
  res.json(imageGallery.map(e => ({ id: e.id, type: e.type, ticker: e.ticker, author: e.author, ts: e.ts })));
});

app.get('/gallery/image/:id', requireAuth, (req, res) => {
  const entry = imageGallery.find(e => e.id === req.params.id);
  ...
  if (isRecap) {
    try {
      ...
      if (originalAlert) {
        ...
        addToGallery('proof', signalTicker, authorName, proofBuf);
