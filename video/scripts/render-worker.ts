...
export function jobPropsToRemotion(job: RenderJob) {
  const { id: _id, composition: _comp, proofImageBase64, templateName, recap_data, props_override, narrationDataUrl, ...rest } = job;
  const templateProps = loadTemplateProps(templateName) || {};
  const override = parsePropsOverride(props_override);
  // narrationDataUrl est ajouté à toutes les compositions ; chacune décide
  // si elle utilise <Audio> selon sa Zod schema.
  const narrationProps = narrationDataUrl ? { narrationDataUrl } : {};
...
async function maybeGenerateNarration(job: RenderJob): Promise<string | null> {
...
async function processJob(
  ...
  // contourner le bundle Remotion snapshot (cf alert PNGs).
  // Best-effort : échec TTS = pas de voix off, mais video render OK.
  const narrationDataUrl = await maybeGenerateNarration(job);
  if (narrationDataUrl) {
    job.narrationDataUrl = narrationDataUrl;
