...
- "entry" : nouveau signal d'entrée (ouvrir une position long/short). Souvent : "$TICKER entry X target Y sl Z", "in $XYZ at 5.50", "long $ABC".
- "exit" : suggestion de sortie d'une position existante. Format compact "TICKER X-Y" sans mots-clés signal, OU phrases comme "trim $XYZ here", "out at 5.50". PAS un status update passé ("PT hit", "stopped out") qui sont à ignorer.
- "ipo" : annonce d'IPO (introduction en bourse). Mentionne "IPO", "raise", "valuation", "price range", "expected to trade".
- "passthrough" : alerte technique brute (volume spike, breakout, news flash) à relayer telle quelle, pas un trade actionnable.
- "ignore" : commentaire, conversation, FYI, status update passé, sans action utile.
...
const FEW_SHOT = [
  ...
  { user: 'VIX spiking, watch SPY',
    assistant: '{"type":"passthrough","ticker":null,"entry":null,"target":null,"stop":null,"low":null,"high":null,"confidence":0.7}' },
...
async function classify(text, { model = DEFAULT_MODEL, timeoutMs = DEFAULT_TIMEOUT_MS } = {}) {
  ...
  try {
    ...
    db.llmClassifyPut(hash, trimmed, parsed.type, parsed, model);
    const latencyMs = Date.now() - t0;
    return { type: parsed.type, entities: parsed, cached: false, latencyMs, model };
  } catch (err) {
    const latencyMs = Date.now() - t0;
    console.warn(
